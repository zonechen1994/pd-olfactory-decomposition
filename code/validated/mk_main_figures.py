...
def pfmt(pv):
    return 'P < 0.001' if pv<0.001 else f'P = {pv:.3f}' if pv<0.1 else f'P = {pv:.2f}'
...
PANEL_CASE=os.environ.get('PANEL_CASE','lower')   # 'upper' for journals that letter panels A, B, C (eBioMedicine)
def panel(ax,l,t=None,dx=-0.16,dy=1.06):
    if PANEL_CASE=='upper': l=l.upper()
...
def fig3():
    ...
    ax.text(1.0,-.255,T('ns: interval includes 0.5. Intervals use 1,000 bootstrap resamples. P values are from Table 3.',
                       'ns:区间含 0.5。柱来自单次 bootstrap。P 值为表 3 的配对比较。'),
            transform=ax.transAxes,ha='right',va='top',fontsize=6.4,color='#777',style='italic')
    panel(ax,'c',T('Seven readings across DAT strata','三种影像情形下七种读法的判别力'),dx=-.09)
