...
**Current analysis:** The corrected comparison and landmark analyses and their aggregate checks are documented in [code/validated/README.md](code/validated/README.md). Run [code/run_analysis.py](code/run_analysis.py) with licensed PPMI inputs to reproduce them. The original scripts and root `results/repro/` remain as a base snapshot; the current pipeline writes its results under `code/validated/results/repro/`. The committed `Source_Data.xlsx` contains the aggregate figure source data.
...
| `code/validated/` | Corrected analyses and checks; see `code/validated/README.md` |
| `data/aggregate_results/` | Aggregate results and checks from the corrected run |
...
> **Code availability.** The analysis scripts, aggregate audit outputs, fitted ridge model and standardisation parameters for the reported analyses are available at https://github.com/zonechen1994/pd-olfactory-decomposition. The model and standardisation parameters allow OIS to be computed in new individuals. An archived version with a DOI will be deposited on acceptance.

## Running

The commands below reproduce the original base analysis. For the current corrected analysis, follow `code/validated/README.md` and use `python code/run_analysis.py` instead.
