...
> **Code availability.** All analyses are implemented as Python scripts that run in a fixed order on the PPMI source files and write every reported value to a numerical ledger, from which the tables, the figures and the Supplementary Information are generated. The scripts, the fitted ridge model with its standardisation parameters and the source data for every figure panel are available at https://github.com/zonechen1994/pd-olfactory-decomposition. The model and its standardisation parameters are what a third party needs to compute OIS in new individuals. An archived version with a DOI will be deposited on acceptance.
